...
public class DeliveryServiceTests
{
    ...
    public async Task GetMyActiveDeliveries_WhenClient_ReturnsOnlyClientActiveDeliveries()
    {
        ...
        _mockDeliveryRepository
            .Setup(r => r.GetUserByKeycloakIdAsync("test"))
        ...
        _mockDeliveryRepository
            .Setup(r => r.GetActiveDeliveriesForUserAsync(
                userId,
                null,
                It.IsAny<IEnumerable<DeliveryStatus>>()))
    ...
    public async Task GetDeliveryDetailAsync_WhenAssignedDriver_ReturnsDetail()
    {
        ...
        _mockDeliveryRepository
            .Setup(r => r.GetDeliveryWithDetailsAsync(deliveryId))
        ...
        var result = await _deliveryService.GetDeliveryDetailAsync(
            deliveryId,
            "test",
    ...
    public async Task GetDeliveryDetailAsync_WhenOtherDriver_ThrowsUnauthorized()
    {
        ...
        _mockDriverRepository
            .Setup(r => r.GetByUserIdAsync(userId))
            .ReturnsAsync(new Driver { Id = otherDriverId });

        await Assert.ThrowsAsync<UnauthorizedAccessException>(() =>
            _deliveryService.GetDeliveryDetailAsync(
                deliveryId,
                "test",
                Enumerable.Empty<string>()));
    }
    ...
    public async Task GetDeliveryDetailAsync_WhenDriverIdEqualsUserId_ThrowsUnauthorized()
    {
        ...
        // This simulates the previous bug where DriverId was compared with UserId.
        // Here, delivery.DriverId equals User.Id, but authorization should only pass if it matches Driver.Id.
        var delivery = new Delivery
        {
            Id = deliveryId,
            DriverId = userId,
