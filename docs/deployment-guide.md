...
### 1. Configuration de Keycloak (`docker-compose.staging.yml`)
- **`KC_HOSTNAME` ne doit pas contenir `/auth`**. Exemple correct : `KC_HOSTNAME=https://${STAGING_DOMAIN}`. Mettre `/auth` dans cette variable entraînerait la génération d'URLs doublées (ex: `/auth/auth/resources/...`) et casserait le chargement du thème et des styles.
...
```bash
curl -k -I "https://staging.lwasel.tech/auth/realms/wasel/protocol/openid-connect/auth?client_id=wasel-front&response_type=code&redirect_uri=http%3A%2F%2Flocalhost%3A8000%2Fendpoint%2Fcallback&scope=openid"
```
